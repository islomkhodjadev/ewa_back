...
async def start(message: types.Message, bot: Bot, state: context.FSMContext) -> None:
    ...
    await message.answer(
        ...
        "Пожалуйста, отправьте логин, который вы указывали при регистрации на сайте ewaproduct.com"
    )
...
async def get_login_password(
    ...
    if status_code == 200:
        ...
        await message.answer(
            "Вы не зарегистрированы на сайте, перейдите по ссылке и зарегистрируйтесь: ewaproduct.com\n"
            "после отправьте логин, который вы указывали при регистрации\n\n"
            "⚠️ Если бот не отвечает после отправки логина, просто перезапустите его командой /start"
        )
        await state.set_state(LoginFSM.access_parameter)
